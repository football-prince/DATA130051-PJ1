...
This pyhton fuctuin implements a dense neural network class.
'''

import numpy as np
from tqdm import tqdm #adds progress bars to various long-running loops
...
class Activator:
    ...
    def derivative(self, name = 'relu'):
        if   name == 'relu':
            ...
            return Activator.derivative_sigmoid
...
class Network():
    ...
    '''
    def __init__(self, hidden_size = [784, 100, 10], acts = ['relu', 'sigmoid'], regws = None, regbs = None, lr = 1e-4):
        '''
        hidden_size:    size of data in hidden layers
        acts:           activator typename
        regws:          l2-regularization penalty on weights
        regbs:          l2-regularization penalty on biases
        lr:             learning rate
        '''
    ...
    def update(self, lr):
        ...
        for i in range(len(self.weights)):
            if self.grads[i] is not None:
                ...
                if self.regws[i] != 0:
                    self.biases[i] -= self.biases[i] * self.regws[i]
    ...
    def compute_loss(self, pred, fact, batch_size, loss_func = 0):
        ...
        if loss_func == 0:   # MSE
            ...
            v = -1. / batch_size * np.sum(fact * np.log(pred) + (1 - fact) * np.log((1 + 2e-7) - pred))
    ...
    def fit(self, x, y, epochs = 1, batch_size = 40, loss_func = 'MSE', valid_x = None, valid_y = None, valid_freq = 1, pre_result = None, accs = None, lr = None):
        ...
        for epoch in range(1, 1 + epochs):
            ...
            for _ in tqdm(range(n // batch_size)):
                ...
                if loss_func == 0:   # MSE
                    ...
                    self.backprop(np.true_divide(predict_y - batch_y, predict_y * ((1 + 1e-5) - predict_y)))
            ...
            if valid_x is not None and epoch % valid_freq == 0:
                # validate
                valid_result = self.predict(valid_x, valid_y, batch_size, loss_func = loss_func, verbose = False)
                accs.append(valid_result[0])
                losses_valid.append(valid_result[1])

                if valid_result[1] < best_loss:  # Check if the current validation loss is better
                    best_loss = valid_result[1]
                    self.save(r'best_model.txt')  # Save the best model weights
    ...
    def predict(self, x, y = None, batch_size = 40, loss_func = None, verbose = True):
        '''Make prediction on x or validate the classification accuracy on (x,y) (if y is given).'''
